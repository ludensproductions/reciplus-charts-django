...
#### 3.2.1 PR quality gates (mandatory)

Before requesting review, ensure all the following are true:

- **No debug / breakpoint artifacts in code**
  - Backend (Python/Django): do not introduce `print(...)`, `breakpoint()`, `pdb`, `ipdb`, `pudb`, `debugpy`, or any interactive debugger hooks/imports.
  - Frontend (JS): do not introduce `console.log(...)`, `console.debug(...)`, `console.trace(...)`, or `debugger;`.

- **Logging policy**
  - If logging is required, use the project’s logging system (e.g., Python `logging`).
  - Logs must be **intentional** and written for a **production purpose** (observability/auditing), not for debugging.
  - Use appropriate log levels (`debug`, `info`, `warning`, `error`) and avoid logging sensitive data.

- **No TODOs in new code**
  - New/modified code must not introduce `TODO`, `FIXME`, or similar placeholders.
  - If work is out of scope, it must be tracked as an **issue/task**, not left as a TODO in code.
